...
export const AllStudentsPageRoot = styled.div`
  ...
  .students-table__points-bar {
    width: 3px;
    border-radius: 999px;
    background: var(--level-color, ${c.orange.soft});
  ...
  .students-table__pill--pending {
    background: ${c.surface.default}af0;
    color: ${c.warning.gold};
  }
  ...
  .students-modal__save:hover {
    background: linear-gradient(135deg, ${c.primary.dark} 0%, ${c.primary.light} 100%);
  }
  ...
  .students-modal__upload {
    ...
    gap: ${theme.spacing(1.75)};
    padding: 24px 22px;
    border: 1.5px dashed ${c.auth.dot};
    border-radius: 24px;
    background:
      radial-gradient(circle at top, ${tokens.rgba.primary_08}, transparent 58%),
      ${c.background.card};
    cursor: pointer;
    text-align: center;
  }
  ...
  .students-modal__cancel {
    border: 1px solid ${c.info.linkBorder};
    background: ${c.surface.default};
    color: ${c.info.link};
  }

  .students-modal__cancel:hover {
    border-color: ${c.info.linkHover};
    background: ${c.info.linkBg};
  }
